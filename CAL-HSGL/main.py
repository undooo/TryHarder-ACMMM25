...
from train_HD import train_cal as train_cal_HD
from train_HD import train_cal_with_memory as train_cal_with_memory_HD
...
def parse_option():
    ...
    parser.add_argument('--extra_data_path', type=str, help="your path to extra data directory")

    
    # loss
    parser.add_argument('--use_triplet_hd',type=str, help="use triplet loss in hard negative mining")
    parser.add_argument('--label_adjust_factor', type=float, default=1.0, help="label adjust factor for triplet loss")
    # Miscs
    ...
    parser.add_argument('--arctype', default='normal', type=str, help='model architecture')
    parser.add_argument('--triplet_w', default=0.0, type=float, help='loss weight of triplet loss')
...
def main(config):
    ...
    for epoch in range(start_epoch, config.TRAIN.MAX_EPOCH):
        ...
        if config.LOSS.USE_TRIPLET_HD == False:
            if config.LOSS.CAL == 'calwithmemory':
            ...
            if config.LOSS.CAL == 'calwithmemory':
                train_cal_with_memory_HD(config, epoch, model, classifier, criterion_cla, criterion_pair, 
                    criterion_adv, optimizer, trainloader, pid2clothes)
            else:
                train_cal_HD(config, epoch, model, classifier, clothes_classifier, criterion_cla, criterion_pair, 
                    criterion_clothes, criterion_adv, optimizer, optimizer_cc, trainloader, pid2clothes)
            ...
            if local_rank == 0:
                if epoch%10 ==0 :
                    save_checkpoint({
                        ...
                        'rank1': rank1,
                        'epoch': epoch,
                    }, is_best, osp.join(config.OUTPUT, 'checkpoint_ep' + str(epoch+1) + '.pth'))
        scheduler.step()
