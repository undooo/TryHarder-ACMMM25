...
def init_fg_cluster(args, model, dataset):
    ...
    if args.dataset in ['ltcc', 'prcc', 'deepchange', 'last', 'vcclothes', 'uccup']:
    ...
    with torch.no_grad():
        dataid = 0
        for data in tqdm(train_loader_normal, position=0, leave=True):
            if args.dataset in ['ltcc', 'prcc', 'deepchange', 'last', 'vcclothes', 'uccup']:
    ...
    # print(feat_dict)
    for pid in trange(len(feat_dict.keys())):
        if len(feat_dict[pid]) == 0:
            print('pid {} has no feature'.format(pid))
    ...
    with torch.no_grad():
        ...
        for dataid, data in enumerate(dataset.train):
            if args.dataset in ['ltcc', 'prcc', 'deepchange', 'vcclothes', 'uccup', 'last']:
                img, pid, _, camid = data
            else:
                img, pid, camid = data
...
def train(args, epoch, dataset, train_loader, model, classifier,
    ...
    for n_iter, data in enumerate(train_loader):
        if args.dataset in ['ltcc', 'prcc', 'deepchange', 'last', 'vcclothes', 'uccup'] or epoch > fg_start_epoch:
        ...
        if epoch > fg_start_epoch and fgid is not None:
        ...
        if fgid is not None and epoch > fg_start_epoch:
            triplet_loss= metric_criterion(feat, pid, hardN_mat, hardP_mat)
            loss += args.trip_weight * triplet_loss
        ...
        if (n_iter + 1) % args.iter_log_period == 0:
                        
            if args.model_name in ['hdetector']:
                print("Epoch[{}] Iteration[{}/{}] Loss: {:.3f} Triplet_loss: {:.3f} class_losses:{:.3f}"
                                    .format(epoch, (n_iter + 1), len(train_loader),loss, triplet_losses.avg, class_losses.avg))
            else:
                print("Epoch[{}] Iteration[{}/{}] Loss: {:.3f}"
                                    .format(epoch, (n_iter + 1), len(train_loader),loss))
