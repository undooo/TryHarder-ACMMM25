...
def train_aim_HD(config, epoch, model, model2, classifier, clothes_classifier, clothes_classifier2, fuse, criterion_cla, criterion_pair,
    ...
    for batch_idx, (imgs, pids, camids, clothes_ids, img_path) in enumerate(trainloader):
        ...
        for i, j in zip(indices[0], indices[1]):
            ...
            if pids[i] != pids[j] and clothes_ids[i] == clothes_ids[j]:
                hardN_mat[i, j] = 1
                hardN_mat[j, i] = 1
            
            # 若两样本id相同，但衣服id不同，则在 hardP_mat 对应位置为 1
            if pids[i] == pids[j] and (clothes_ids[i] != clothes_ids[j] or camids[i] != camids[j]):
                hardP_mat[i, j] = 1
                hardP_mat[j, i] = 1
        ...
        pair_loss = criterion_pair(features, pids, hardN_mat, hardP_mat)
