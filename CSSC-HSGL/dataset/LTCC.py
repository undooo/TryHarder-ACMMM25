...
class LTCC(BaseImageDataset):
    ...
    """
    def __init__(self, dataset_root='data', dataset_filename='LTCC_ReID',cfg=None, verbose=True, **kwargs):
        self.dataset_dir = '/root/autodl-tmp/LTCC_ReID'
        self.train_dir = osp.join(self.dataset_dir, 'train')
        self.query_dir = osp.join(self.dataset_dir, 'query')
        self.gallery_dir = osp.join(self.dataset_dir, 'test')
        self._check_before_run()
        self.extra_data_path = cfg.extra_data_path
        self.cfg = cfg
    ...
    def _process_dir_train(self, dir_path):
        ...
        if self.extra_data_path is not None:
            extra_img_paths = glob.glob(osp.join(self.extra_data_path, '*.png'))
        ...
        inpaint_path = "/root/autodl-tmp/ltcc-inpaint/ltcc_inpaint"
        if self.cfg.use_inpaint == "True":
            for i in range(1, 9):
                cur_dir =osp.join(inpaint_path, f"epoch{i}","train")
                cur_img_path = glob.glob(osp.join(cur_dir,"*.png"))
                img_paths.extend(cur_img_path)
        for img_path in img_paths:
